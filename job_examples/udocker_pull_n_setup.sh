...
DEFAULTIMG="vykozlov/tf-benchmarks:latest-gpu"
UCONTAINER="tfbench-gpu"
####

UDOCKERSETUP="--execmode=F3 --nvidia"
...
if [ ${#IFExist} -le 1 ]; then
    ...
    if (udocker create --name=${UCONTAINER} ${DOCKERIMG}); then
       echo "Now setting up container $UCONTAINER with options: $UDOCKERSETUP"
